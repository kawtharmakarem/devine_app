...
class _HoroscopeScreenState extends State<HoroscopeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Padding(padding: EdgeInsets.symmetric(horizontal: width/20),
      child: SingleChildScrollView(
        physics:const BouncingScrollPhysics(),
        child:width<AppConstants.maxMobileWidth? getMobileHoroscopeContent():getTabletHoroscopeContent(),),),
    );
  }
  ...
  Column getMobileHoroscopeContent() {
    return Column(
        children: [
          ...
          return HoroscopeItem(index: index,horoscopeModel:horoscopes[index] ,onTap: (){
            Navigator.of(context).push(MaterialPageRoute(builder: (context)=>HoroscopeDetailsScreen(horoscopeModel: horoscopes[index])));
          },);
          })
        ],
      );
